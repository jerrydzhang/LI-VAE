...
def test_lowpass_reduces_high_frequency_content() -> None:
    ...
    filtered = filt.lowpass_filter(checkerboard, cutoff_radius=5)
...
def test_highpass_preserves_high_frequency_content() -> None:
    ...
    filtered = filt.highpass_filter(checkerboard, cutoff_radius=5)
...
def test_bandpass_removes_low_frequency_background() -> None:
    ...
    filtered = filt.bandpass_filter(combined, low_cutoff=5, high_cutoff=20)
...
def test_bandpass_invalid_cutoffs() -> None:
    ...
    with pytest.raises(ValueError):
        filt.bandpass_filter(img, low_cutoff=10, high_cutoff=5)


def test_input_validation_non_2d() -> None:
    img3d = np.zeros((2, 2, 2))
    with pytest.raises(ValueError):
        filt.lowpass_filter(img3d, cutoff_radius=5)
