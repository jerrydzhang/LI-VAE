...
def run_training(args: argparse.Namespace) -> None:
    """Main training loop for plain VAE.
    ...
    print(f"Model initialized: VAE with {args.latent_dim}-dim latent space")
...
def build_argparser() -> argparse.ArgumentParser:
    """Build argument parser for VAE training script."""
    parser = argparse.ArgumentParser(
        description="Train standard VAE on atom patches from STEM microscopy"
    )
    ...
    parser.add_argument(
        "--log-dir", type=str, default="runs/vae", help="TensorBoard log directory"
    )
    parser.add_argument(
        "--vis-every",
        type=int,
