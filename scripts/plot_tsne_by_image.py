...
def collect_stats(model, loader, dataset):
    ...
    for batch in loader:
        ...
        err = F.mse_loss(recon, x, reduction='none').mean(dim=(1,2,3)).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    dataset = AdaptiveLatticeDataset(images, patch_size=patch_size, padding=16, transform=None)
    ...
    for i, img_idx in enumerate(np.unique(image_sources)):
        ...
        plt.scatter(emb_src[mask,0], emb_src[mask,1], s=8, color=cmap(i % 10), label=f'Image {img_idx} (n={mask.sum()})', alpha=0.8)
